...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if coin<epsilon:
        ...
        (action_index , object_index) = np.unravel_index(np.argmax(q_func[state_1,state_2]), q_func.shape[2:])
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    if terminal:
        ...
        q_values_next_state = np.max(q_func[next_state_1,next_state_2,:,:])
        max_next_state = np.max(q_values_next_state)
    q_current = q_func[current_state_1,current_state_2,action_index,object_index]
    q_func = (1-ALPHA)*q_current+ALPHA*(reward + GAMMA*max_next_state)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            ...
            tabular_q_learning(q_func, room_index, quest_index, action_index,object_index, reward, next_state_1, next_state_2,terminal)
